...
"_______________________________________________________________________________________________________
	...
	:  autocmd BufNewFile *.cs :execute "normal! iclass ".expand('%')[:-4]."\n{\n}\<esc>O_\<BS>"
	:augroup END
	" }}}
	...
	:  autocmd FileType vim :inoremap <silent><buffer><tab> <C-R>=CleverTab()<CR>
	:augroup END
	" }}}
	...
	:autocmd BufRead,BufNewFile *.notes.md :cabbrev markdown call NotesMDFormat()
	:augroup END
	" }}}
